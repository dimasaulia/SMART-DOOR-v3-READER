...
boolean WiFiManager::autoConnect(char const *apName, char const *apPassword) {
  ...
  if(getWiFiIsSaved()){
    ...
    if(connected || connectWifi(_defaultssid, _defaultpass) == WL_CONNECTED){
      ...
      Serial2.print("DIS#CON!");
      Serial.print("DIS#CON!");
      #endif
...
#ifdef WM_DEBUG_LEVEL
  Serial.print("DIS#DCN!"); Serial2.print("DIS#DCN!");
...
String WiFiManager::WiFi_psk(bool persistent) const {
  ...
  void WiFiManager::WiFiEvent(WiFiEvent_t event,system_event_info_t info){
    ...
    if(event == ARDUINO_EVENT_WIFI_STA_DISCONNECTED){
      ...
      #ifdef WM_DEBUG_LEVEL
      if(info.wifi_sta_disconnected.reason == WIFI_EVENT_STA_DISCONNECTED || info.wifi_sta_disconnected.reason == WIFI_REASON_UNSPECIFIED || info.wifi_sta_disconnected.reason == WIFI_REASON_AUTH_LEAVE || info.wifi_sta_disconnected.reason == WIFI_REASON_ASSOC_EXPIRE) {Serial.print("DIS#DCN!"); Serial2.print("DIS#DCN!");};
      if(info.wifi_sta_disconnected.reason == WIFI_REASON_NO_AP_FOUND) {DEBUG_WM(DEBUG_VERBOSE,F("[EVENT] WIFI_REASON: NO_AP_FOUND")); Serial.print("DIS#DCN!"); Serial2.print("DIS#DCN!");};
